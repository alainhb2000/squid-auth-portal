...
module.exports = function (options) {
    ...
    function ldap_getGroupMembersAsync(dn) {
        return new Promise((resolve, reject) => {
            ldap_search(dn, {
                scope: 'sub',
                ...
                if (ldapResult && ldapResult.length >= 1) {
                    if (options.activeDirectory) {
                        if (ldapResult[0].member)
                            return resolve(ldapResult[0].member);
                    } else {
                        if (ldapResult[0].memberUid)
                            return resolve(ldapResult[0].memberUid);
                    }
                }
    ...
    return {
        ...
        getGroupMembersAsync: ldap_getGroupMembersAsync,
